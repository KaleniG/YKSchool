document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll<HTMLTableRowElement>("table.edit tr[data-id]")?.forEach((row) => {
    ...
    const teachingSubjectsInput = row.querySelector<HTMLSelectElement>(`select[name="operation[save][${id}][teaching_subjects][]"]`);
    ...
    const sendData = async () => {
      ...
      Array.from(teachingSubjectsInput.selectedOptions).forEach(opt =>
        formData.append(`operation[save][${id}][teaching_subjects][]`, opt.value)
      );
